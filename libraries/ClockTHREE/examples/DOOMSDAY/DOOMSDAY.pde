...
void setup(){
  ...
  target_tm.Year = year();
  target_tm.Month = month();
  target_tm.Day = day();
  target_tm.Hour = hour();
  target_tm.Minute = minute();
  target_tm.Second = second();
  target = 10; //makeTime(target_tm);
  Serial.print("Seconds to go: ");
  Serial.println(target - now());
}
...
void loop(){
  ...
  Serial.print("hh togo: ");
  Serial.println(hh_togo);
